...
try:
    if(len(sys.argv) > 2):
        testrunner_to_run : str = sys.argv[2]
        print("TestRunner to run : " + testrunner_to_run)
        run_shell_command(f"mvn clean install -P e2e-tests -pl {module_to_build} -DTEST_RUNNER={testrunner_to_run}")
    else:
        run_shell_command(f"mvn clean install -P e2e-tests -pl {module_to_build}")
